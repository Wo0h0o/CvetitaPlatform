// ISO 3166-1 country code utilities for the world-map view.
//
// The world-atlas topojson keys countries by ISO numeric (BG = "100",
// GR = "300", RO = "642") while our sales data uses ISO alpha-2 (BG, GR,
// RO). This file bridges the two and provides Bulgarian country names
// for tooltips on markets we care about.
//
// Coverage: full 249-country ISO 3166-1 map (so any country in the
// topojson can be matched), plus БГ names for ~60 high-relevance
// markets (EU + neighbours + common e-commerce destinations). Countries
// without a БГ entry fall back to the English name from topojson
// properties — still readable, just non-localised.
...
// Reverse lookup — alpha-2 → numeric. Built once at module load.
export const ALPHA2_TO_ISO_NUMERIC: Record<string, string> = Object.fromEntries(
  Object.entries(ISO_NUMERIC_TO_ALPHA2).map(([num, alpha2]) => [alpha2, num])
);

// БГ names for high-relevance markets. Add to this list as we expand;
// missing entries fall back to topojson's English `properties.name`.
export const ALPHA2_TO_NAME_BG: Record<string, string> = {
  BG: "България",
  GR: "Гърция",
  RO: "Румъния",
  RS: "Сърбия",
...
/**
 * Resolve a topojson numeric id to its ISO alpha-2 code, or null when
 * the id is unknown (the 110m world atlas has a handful of disputed
 * territories without official ISO numeric codes).
 */
export function numericToAlpha2(numeric: string | number | undefined): string | null {
...
/**
 * Display name for a country — БГ when known, otherwise the English
 * name passed from topojson properties.
 */
export function countryDisplayName(alpha2: string | null, fallback: string): string {
  if (alpha2 && ALPHA2_TO_NAME_BG[alpha2]) return ALPHA2_TO_NAME_BG[alpha2];
  return fallback;
}
