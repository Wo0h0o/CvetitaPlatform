...
/**
 * Продуктов списък с избор → създаване на възлагателно писмо.
 * Ползва се и за „Цветита Хербал" (СТОКИ), и за „Ишлемета".
 */
export function ProductionSelector({
  ...
  const { data: ordersData } = useSWR<{
    orders: { id: number; letter_no: string | null; issued_date: string; status: string; items: { item_id: string; status?: string; produced_qty?: number | null; qty: number }[] }[];
  ...
  for (const o of ordersData?.orders ?? []) {
    ...
    for (const it of o.items) {
      const prod = it.produced_qty != null ? it.produced_qty : it.status === "produced" ? it.qty : 0;
      if (prod >= it.qty) continue;
      pendingInOrder[String(it.item_id)] = { label: o.letter_no ? `№${o.letter_no}` : `#${o.id}`, date: o.issued_date };
    }
  }
    ...
    <div className="space-y-6">
      {showBuckets && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <SummaryCard label="Критично ≤30д" value={buckets.crit} tone="crit" />
          <SummaryCard label="Заяви сега 31–60д" value={buckets.order} tone="order" />
          <SummaryCard label="Наблюдавай 61–90д" value={buckets.watch} tone="watch" />
          <SummaryCard label="ОК >90д" value={buckets.ok} tone="ok" />
        </div>
      )}
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between flex-wrap gap-3">
          <div>
            <h3 className="text-[15px] font-semibold text-text">{listTitle}</h3>
            ...
            <div className="flex items-center gap-1 text-[12px]">
              ...
              {([
                ...
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg cursor-pointer transition-colors ${sortBy === key ? "bg-accent-soft text-accent font-medium" : "text-text-2 hover:bg-surface-2"}`}
                >
                  {label}
                </button>
              ))}
            </div>
            <button onClick={suggestAll} className="text-[12px] px-3 py-1.5 rounded-lg border border-border text-text-2 hover:bg-surface-2 cursor-pointer">
          ...
          <table className="w-full text-[13px] min-w-[900px]">
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className={`border-t border-border border-l-2 ${STRIPE[s.k]} ${checked ? "bg-accent-soft/40" : ""}`}>
                    <td className="px-3 py-2.5 text-center">
                      <input type="checkbox" checked={checked} onChange={() => toggle(r)} className="w-4 h-4 accent-[var(--accent)] cursor-pointer" aria-label={`Избери ${r.name}`} />
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right tabular-nums text-[15px] font-bold text-text">{r.cover > 900 ? ">900" : nf(r.cover)}</td>
                    <td className="px-4 py-2.5">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {checked ? (
                        <input type="number" min={0} step={50} value={sel[r.id]} onChange={(e) => setQty(r.id, parseInt(e.target.value || "0", 10))} className="w-24 px-2 py-1 text-right rounded-md border border-border bg-surface tabular-nums" />
                      ) : (
                        <span className="tabular-nums text-accent font-semibold">{(r.prodQty ?? 0) > 0 ? nf(r.prodQty!) : "—"}</span>
                      )}
                    </td>
                  </tr>
                );
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 md:left-[var(--sidebar-width)] bg-surface border-t border-border px-5 py-3 flex items-center justify-between gap-3 z-30 shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
          <div className="text-[13px] text-text-2">
            Избрани <b className="text-text">{selectedIds.length}</b> продукта · {nf(selectedIds.reduce((a, id) => a + (sel[id] || 0), 0))} бр общо
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setSel({})} className="text-[13px] px-3 py-2 rounded-lg text-text-3 hover:bg-surface-2 cursor-pointer">Изчисти</button>
            <button onClick={createZayavka} className="flex items-center gap-2 text-[13px] font-medium px-4 py-2 rounded-lg bg-accent text-white hover:opacity-90 cursor-pointer">
