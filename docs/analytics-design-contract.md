...
| **Две метрики като диагностична позиция** + трета като тежест? (внимание × конверсия × приходи на продукт) | **Bubble quadrant** | Позицията декларира диагноза (кой от 4 квадранта); размерът дава тежест без да претрупва — голям обект = голям балон, опашката = спокоен облак | `<ScatterChart>` + `<Scatter>` + `<ZAxis range>` за размер |
...
- **Bubble quadrant** колапсва до квадрант-групиран списък под `md:` (виж §9.7)

#### 9.7 Bubble quadrant — кога и дисциплина

Диагностична карта: две метрики позиционират обекта в един от 4 квадранта (внимание × конверсия → звезда / изтичащ / скрита перла / спящ), трета метрика (приходи) е размерът на балона. Сигнатурният изглед на `/products`.

- ✅ Когато въпросът е „**каква е формата на каталога ми**" — кои обекти изтичат, кои са скрити възможности. Диагноза, не ранкинг.
- **Мащабът се носи от размера, не от пагинация.** Всички квалифициращи обекти на една карта; едрите доминират, дългата опашка е спокоен облак от малки точки. Не режи чарта на страници — това убива bird's-eye-а. (Пагинацията живее в придружаващия списък, не в чарта.)
- **Праг за минимум данни.** Обект под N наблюдения (шумно съотношение — 2 сесии / 1 покупка = 50%) **не се чертае**; отива в отделен „Недостатъчно данни" бакет. Иначе квадрантът лъже.
- **Маркерите са точки — без снимки, без текст по тях.** Само позиция + размер + цвят (§10 marker дисциплина). Топ ~6 по тежест получават статично **halo-glow** (§10 top-performer halo), НЕ текстов етикет — текстовите етикети се застъпват в нечетима купчина на гъст каталог; идентичност идва от hover или списъка. Принцип: чартът казва повече с по-малко — визуално кодиране, не добавен текст.
- **Цвят = диагноза, не категория.** Квадрантите носят good/bad смисъл → accent / red / neutral (§1). Никакви 4 категорийни цвята.
- **Тинтът на квадранта носи теглото.** Hue = диагноза (горе); fill-opacity ∝ дял от оборота на квадранта — money-heavy квадрантът свети най-силно, центърът на тежестта на каталога се чете без verdict ред или числа.
- **Hover → glass pop-up (§11)** с идентичност (снимка, име) + метрики + „защо е тук".
- **Mobile** — scatter + hover не работят на тъч (§13); под `md:` колапсва до квадрант-групиран списък („Изтичащи (8)" разгъваеми секции).
- Разделящите оси седят на дефиниран праг (медиана или бизнес-праг), не на 0.
